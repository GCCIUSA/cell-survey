  ...
  */
  getNodeByUid(uid) {
    ...
    this._ref.orderByKey().once("value", (snapshot) => {
      let nodes = this._parse(snapshot.val()).filter(node => {
        return node.leaders && node.leaders.indexOf(uid) >= 0;
      });
  ...
  */
  getChildren(node) {
    ...
    this._ref.orderByChild("depth").equalTo(node.depth + 1).once("value", (snapshot) => {
      let children = this._parse(snapshot.val()).filter(item => {
        return item.path.substr(0, node.path.length) === node.path;
      });
  ...
  */
  getDescendants(node, levels) {
    ...
    this._ref.orderByChild("depth").startAt(node.depth + 1).once("value", (snapshot) => {
      let descendants = this._parse(snapshot.val()).filter(item => {
        return (!levels || levels.indexOf(item.level) >= 0) && item.path.substr(0, node.path.length) === node.path;
      });
  ...
  */
  getSiblings(node, side, inclSelf) {
    ...
    this._ref.orderByChild("depth").equalTo(node.depth).once("value", (snapshot) => {
      let nodeIndex = this._getNodeIndex(node);

      let siblings = this._parse(snapshot.val()).filter(item => {
        return this._getParentPath(item) === this._getParentPath(node);
      });
      this._sort(siblings);
